...
var $ReC={};
(function(){'use strict';
$ReC.isIE=function(){var ie=/*@cc_on!@*/false;if(ie){return true;}else{return false;}};
if($ReC.isIE()){alert('Doesn\'t correspond to the old IE. Please use the HTML5 browser.');return;}
/* basic */
$ReC.doc     =document;
$ReC.id      =function(i){return $ReC.doc.getElementById(i);};
...
$ReC.toInt   =function(tx,df){if(!$ReC.isStrBlk(tx)){try{var n=parseInt(tx);if(!isNaN(n)){return n;}}catch(e){}}if($ReC.isNum(df)){return df;} return null;};
$ReC.toFloat =function(tx,df){if(!$ReC.isStrBlk(tx)){try{var n=parseFloat(tx);if(!isNaN(n)){return n;}}catch(e){}}if($ReC.isNum(df)){return df;} return null;};
$ReC.toDate  =function(tx,df){if(!$ReC.isStrBlk(tx)){try{var d=new Date(tx);if(d.toString()!=="Invalid Date"){return d;}}catch(e){}}if($ReC.isDate(df)){return df;} return null;};
$ReC.parseJSON =function(tx){if(!$ReC.isStrBlk(tx)){try{return JSON.parse(tx);}catch(e){}} return null;};
$ReC.escapeHTML =function(tx){if(!$ReC.isStrBlk(tx)){tx=tx.replace(/[ &'`"<>]/g,function(match){return {' ':'&nbsp;','&':'&amp;',"'":'&#x27;','`':'&#x60;','"':'&quot;','<':'&lt;','>':'&gt;'}[match]});return tx.replace(/\r?\n/g,'<br />');} return '';};
})();
