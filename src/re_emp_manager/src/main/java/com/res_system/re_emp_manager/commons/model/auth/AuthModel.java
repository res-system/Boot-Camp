   ...
   /**
    ...
    public boolean doLogin(
        ...
        if (!ReUtil.isEmpty(group_code)) {
            result = dao.find(AuthMUser.class, "find_by_group_has_account"
                    , (st)->{
            ...
            result = dao.find(AuthMUser.class, "find_by_account"
                    , (st)->{
                        st.setString(1, UsrKbn.NORMAL.getValue()); 
    ...
    public boolean doSystemLogin(final String login_id, final String password) 
        ...
        // -- ログインアカウント情報取得.
        AuthMUser result = dao.find(AuthMUser.class, "find_by_account"
                    , (st)->{
                        st.setString(1, UsrKbn.SYSTEM.getValue()); 
                        st.setString(2, login_id); 
                    });
        if (result != null) {
            // -- パスワードチェック.
    ...
    public boolean changeGroupId(final String group_id) throws SQLException {
        ...
        if (result != null) {
            this.authTLogin.setUser_id(result.getUser_id());
            this.authTLogin.setGroup_id(result.getGroup_id());
            this.authTLogin.setAuthority_id(result.getAuthority_id());
            // ログイン情報更新.
     ...
     * @throws SimpleDaoException
     */
    private boolean saveLoginToken(final AuthMUser user, final boolean isSaveAuth) throws SimpleDaoException {
        if (user == null) {
            return false;
        }
     ...
     * @throws SQLException
     */
    private boolean loadLoginInfo() throws SimpleDaoException, SQLException {
        this.authTLogin = null;
