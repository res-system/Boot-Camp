...
public class ChangeGroupModel implements IMessage {
    ...
    //---------------------------------------------- const [private].
    /** 処理名. */
    private static final String PROC_NAME = "グループの変更";
     ...
     * @throws SimpleDaoException
     */
    private List<AuthRGrpMember> getListData(final SearchCondition searchCond) throws SimpleDaoException {
        return common.getListData(AuthRGrpMember.class, searchCond
                , dao.getSql(AuthRGrpMember.class, "find_list")
                , common.makeWhere(searchCond, "")
                , getOrder(searchCond)
    ...
    private String getOrder(final SearchCondition searchCond) throws SimpleDaoException {
        ...
        if (AuthRGrpMember.SORT_MIN <= sort && sort <= AuthRGrpMember.SORT_MAX) {
            order = dao.getSql(AuthRGrpMember.class, "find_list_order_" + searchCond.getSort());
        } else {
            order = dao.getSql(AuthRGrpMember.class, "find_list_order");
        }
        return order;
    }
