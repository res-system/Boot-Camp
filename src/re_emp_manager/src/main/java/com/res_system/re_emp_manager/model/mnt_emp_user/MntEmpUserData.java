...
@Sqls({
    @Sql(name="find_list", sql = ""
            ...
            + ",(CASE WHEN `ACC`.`login_id` IS NOT NULL THEN '1'"
            +       " WHEN `GRPAC`.`login_id` IS NOT NULL THEN '1'"
            ...
            + ",`PGRP`.`id` AS `possess_group_id`"
            + ",(CASE WHEN `PGRP`.`id` IS NOT NULL THEN '1'"
            ...
            +   " INNER JOIN `m_group` `GRP` ON `GRP`.`id` = `GRPAC`.`default_group_id`"
            ...
            +   " LEFT OUTER JOIN `m_group` `PGRP`"
            +       " ON `PGRP`.`id` = `GRPAC`.`root_group_id` AND `PGRP`.`owner_account_id` = `GRPAC`.`account_id`"
            +   " LEFT OUTER JOIN `m_account` `ACC` ON `ACC`.`id` = `GRPAC`.`account_id`"
    ...
    ,@Sql(name="check_owner", sql = ""
            ...
            +   " INNER JOIN `m_account` `ACC` ON `ACC`.`id` = `GRP`.`owner_account_id`"
            + " WHERE `GRPAC`.`root_group_id` = ?"
            +   " AND `GRPAC`.`user_id` = ?"
            )
...
public class MntEmpUserData implements IEntity {
