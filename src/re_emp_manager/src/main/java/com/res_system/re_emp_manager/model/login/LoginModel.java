...
public class LoginModel implements IMessage {
    ...
    public boolean checkInput(final LoginForm form) {
        ...
        selector = "#id";
        value = form.getId();
        if (!checker.checkLoginId(value, true, name, selector)) { ret = false; }
        //----------------
        name = "パスワード";
        selector = "#key";
        value = form.getKey();
        if (!checker.checkPassword(value, true, name, selector)) { ret = false; }
    ...
    public boolean doLogin(final LoginForm form) throws Exception {
        try {
            ...
            if (auth.doLogin(form.getCode(), form.getId(), form.getKey(), !ReUtil.isEmpty(form.getSave()))) {
                // 認証OK.
                dao.commit();
                addMessage(msgModel.getMessage("I01001"));
                form.setNext(NEXT);
                ...
                // 認証NG.
                dao.rollback();
                addMessage(msgModel.getMessage("E01001").addSelector("#id").addSelector("#key"));
                return false;
            }
