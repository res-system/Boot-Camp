...
@Sqls({
    @Sql(name="find_list", sql = ""
            ...
            +       " ON `ACC`.`id` = `GRPAC`.`account_id` AND `ACC`.`acc_status` IN('0', '1')"
            +   " LEFT OUTER JOIN `m_user` `AUSR` ON `AUSR`.`id` = `ACC`.`user_id`"
            + " WHERE `USR`.`id` = ?"
            + " UNION "
            + "SELECT "
            + " `GRP`.`id` AS `id`"
            ...
            +       " ON `ACC`.`id` = `GRPAC`.`account_id` AND `ACC`.`acc_status` IN('0', '1')"
            +   " INNER JOIN `m_user` `AUSR` ON `AUSR`.`id` = `ACC`.`user_id`"
            + " WHERE `GRPAC`.`account_id` = ?"
            )
    ,@Sql(name="find_list_order", sql = "ORDER BY `created` DESC ")
    ,@Sql(name="find_list_order_1", sql = "ORDER BY `root_group_name` ASC ")
    ,@Sql(name="find_list_order_2", sql = "ORDER BY `root_group_name` DESC ")
    ,@Sql(name="find_list_order_3", sql = "ORDER BY `group_name` ASC ")
    ,@Sql(name="find_list_order_4", sql = "ORDER BY `group_name` DESC ")
    ,@Sql(name="find_list_order_5", sql = "ORDER BY `user_name` ASC ")
    ,@Sql(name="find_list_order_6", sql = "ORDER BY `user_name` DESC ")
    ,@Sql(name="find_list_order_7", sql = "ORDER BY `authority_name` ASC ")
    ,@Sql(name="find_list_order_8", sql = "ORDER BY `authority_name` DESC ")
    ,@Sql(name="find_list_order_9", sql = "ORDER BY `created` ASC ")
    ,@Sql(name="find_list_order_10", sql = "ORDER BY `created` DESC ")
