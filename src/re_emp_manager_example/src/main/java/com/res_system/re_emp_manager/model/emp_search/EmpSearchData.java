...
@Sqls({
    ...
    ,@Sql(name="find_list_order", sql = "ORDER BY `group_id`, `authority_id`, `employee_no` ")
    ,@Sql(name="find_list_order_1", sql = "ORDER BY `group_name` ASC ")
    ,@Sql(name="find_list_order_2", sql = "ORDER BY `group_name` DESC ")
    ,@Sql(name="find_list_order_3", sql = "ORDER BY `authority_id` ASC ")
    ,@Sql(name="find_list_order_4", sql = "ORDER BY `authority_id` DESC ")
    ,@Sql(name="find_list_order_5", sql = "ORDER BY `employee_no` ASC ")
    ,@Sql(name="find_list_order_6", sql = "ORDER BY `employee_no` DESC ")
    ,@Sql(name="find_list_order_7", sql = "ORDER BY `name` ASC ")
    ,@Sql(name="find_list_order_8", sql = "ORDER BY `name` DESC ")
    ,@Sql(name="find_list_order_9", sql = "ORDER BY `sex` ASC ")
    ,@Sql(name="find_list_order_10", sql = "ORDER BY `sex` DESC ")
    ,@Sql(name="find_list_order_11", sql = "ORDER BY `birthday` ASC ")
    ,@Sql(name="find_list_order_12", sql = "ORDER BY `birthday` DESC ")
    ,@Sql(name="find_list_order_13", sql = "ORDER BY `situation` ASC ")
    ,@Sql(name="find_list_order_14", sql = "ORDER BY `situation` DESC ")
    ,@Sql(name="find_list_order_15", sql = "ORDER BY `hire_date` ASC ")
    ,@Sql(name="find_list_order_16", sql = "ORDER BY `hire_date` DESC ")
    ,@Sql(name="find_list_order_17", sql = "ORDER BY `retirement_date` ASC ")
    ,@Sql(name="find_list_order_18", sql = "ORDER BY `retirement_date` DESC ")
    ,@Sql(name="find_list_order_19", sql = "ORDER BY `memo` ASC ")
    ,@Sql(name="find_list_order_20", sql = "ORDER BY `memo` DESC ")
...
public class EmpSearchData implements IEntity {
    ...
    /** 並び順 [最大]. */
    public static final int SORT_MAX = 20;
